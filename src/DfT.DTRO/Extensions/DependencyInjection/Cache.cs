...
{
    /// <summary>
    /// Adds an <see cref="IDtroCache"/> based on the provided configuration.
    /// </summary>
    /// <param name="services">The <see cref="IServiceCollection"/> to add the service to.</param>
    /// <param name="configuration">The cache configuration.</param>
    /// <returns>A reference to this instance after the operation has completed.</returns>
    public static IServiceCollection AddDtroCache(this IServiceCollection services, IConfiguration configuration)
    {
        return configuration.GetValue("EnableRedisCache", false)
            ? services.AddRedisDtroCache(configuration)
            : services.AddSingleton<IDtroCache, NoopDtroCache>();
    }

    /// <summary>
    /// Adds an <see cref="IDtroCache"/> with a Redis backend.
    /// </summary>
    /// <param name="services">The <see cref="IServiceCollection"/> to add the service to.</param>
    /// <param name="configuration">The configuration to get connection parameters from.</param>
    /// <returns>A reference to this instance after the operation has completed.</returns>
    public static IServiceCollection AddRedisDtroCache(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        return services.AddRedisDtroCache(host, password, port, useSsl, certificateAuthorityCertPath);
    }
    ...
    /// <returns>A reference to this instance after the operation has completed.</returns>
    public static IServiceCollection AddRedisDtroCache(
        ...
        string certificateAuthorityCertPath = null)
    {
        return services
            .AddStackExchangeRedisCache(opt =>
            {
                ...
                {
                    EndPoints = { $"{host ?? throw new ArgumentNullException(nameof(host))}:{port}" }, Ssl = useSsl
                };
            ...
            .AddScoped<IDtroCache, DistributedDtroCache>();
    }
